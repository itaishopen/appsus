    ...
    methods: {
        setRating($event, persist) {
            if (!this.readOnly) {
                ...
                } else this.$emit('current-rating', this.currentRating);
            }
        },
        resetRating() {
            if (!this.readOnly) {
        ...
        createStars(round = true) {
            ...
            for (var i = 0; i < this.maxRating; i++) {
                let level = 0
                if (i < this.currentRating) level = (this.currentRating - i > 1) ? 100 : (this.currentRating - i) * 100;
